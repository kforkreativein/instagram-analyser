...
import { buildViralScorePrompt } from "@/lib/viral-prompts";
...
/** Gemini JSON mode rejects some preview IDs; never pass a non-Gemini id to the Gemini client. */
...
export async function POST(req: NextRequest) {
  ...
  const prompt = buildViralScorePrompt({ script, topic, angle, clientProfile });
  ...
  try {
    ...
    return NextResponse.json(
      { error: msg.includes("JSON") ? `Scoring parse error: ${msg}` : msg },
      { status: 500 }
    );
  }
}
