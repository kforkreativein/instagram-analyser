...
import {
  buildNumberedSpokenScriptForPacing,
  clampPacingSegment,
  scriptBodyForAuxiliaryAI,
} from "@/lib/script-pacing-utils";
    ...
    switch (action) {
      case 'improve':
        if (pacingAnalysis) {
          ...
          systemPrompt = `${customDirectives}Rewrite this script for stronger retention. Make it sound natural and human; remove generic AI phrasing. Keep the same language/tone as the input. CRITICAL: Preserve every bracketed structure tag (e.g. [HOOK], [BODY], [CALL TO ACTION]) on its own line — same tags, same order. You may tighten or clarify wording; total length should stay within roughly ±15% of the original word count (do not balloon into a new script). Return ONLY the rewritten script.${criticalInstructions}\n\nScript:\n${script}`;
        }
        break;
      case 'pacing': {
        const { lines, numbered } = buildNumberedSpokenScriptForPacing(script);
        const maxLine = Math.max(1, lines.length);
        systemPrompt = `Analyze this script's pacing as a Retention Engineer.

IMPORTANT: Line numbers refer ONLY to "spoken lines" below. Ignore entirely: blank lines, lines that are ONLY a bracketed label like [HOOK] or [BODY], lines that are only "Quick recap" / "Hook + CTA" / recap meta labels.

Spoken lines (numbered 1..${maxLine}):
${numbered || "(empty)"}
...
Rules:
- Every lineStart and lineEnd MUST be between 1 and ${maxLine} inclusive.
- Merge adjacent slow zones when possible; avoid more than 12 segments.
Status must be one of: "Good", "Slow", "Critical"`;
        break;
      }
      case 'visuals': {
        const bodyOnly = scriptBodyForAuxiliaryAI(script);
        systemPrompt = `Analyze this script and produce a B-Roll cue list for every 5-second segment. Use ONLY this markdown format, one entry per line:\n- **0:00-0:05:** Description of the visual action or B-roll.\n\nDo not use tables. Do not add any extra text. Return ONLY the markdown list.\n\nScript (spoken lines only, no section labels):\n${bodyOnly}`;
        break;
      }
      case 'prompts': {
        const bodyOnly = scriptBodyForAuxiliaryAI(script);
        systemPrompt = `Generate ONE Midjourney/Flux image prompt and ONE video (Runway/Kling-style) prompt per full spoken SENTENCE. A sentence ends at . ! or ? (treat each full sentence as one unit — never split mid-sentence for separate prompts).

Script (spoken content, labels removed):\n${bodyOnly}

Return a numbered list in plain text:
1. [Sentence] — Image: ... — Video: ...`;
        break;
      }
      case 'caption': {
        const bodyOnly = scriptBodyForAuxiliaryAI(script);
...
[{"title": "Fix X", "suggestion": "Do Y because Z", "impact": "High"}]`;
  ...
  5. DO NOT output visual cues. Only output the spoken script.`;
    ...
    if (action === "pacing") {
      const maxLine = Math.max(1, buildNumberedSpokenScriptForPacing(script).lines.length);
      try {
        const firstBrace = output.indexOf("{");
        const lastBrace = output.lastIndexOf("}");
        if (firstBrace >= 0 && lastBrace > firstBrace) {
          const parsed = JSON.parse(output.slice(firstBrace, lastBrace + 1)) as {
            segments?: Array<{ lineStart: number; lineEnd: number; status: string; note: string }>;
            summary?: string;
          };
          if (Array.isArray(parsed.segments)) {
            parsed.segments = parsed.segments.map((s) => clampPacingSegment(s, maxLine));
          }
