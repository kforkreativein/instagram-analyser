...
function HomePageContent() {
  ...
  useEffect(() => {
    ...
    const nextSettings = parseLocalSettings(localStorage.getItem(LOCAL_SETTINGS_KEY));
    const savedOpenAI = (localStorage.getItem("OPENAI_API_KEY") || "").trim();
    const savedGemini = (localStorage.getItem("GEMINI_API_KEY") || "").trim();
    const savedEngine = (localStorage.getItem("analysisEngine") || "").trim().toLowerCase();
    const savedApify = (localStorage.getItem("APIFY_API_KEY") || "").trim();

    setSettings({
      ...nextSettings,
      aiProvider: savedEngine === "gemini" ? "gemini" : nextSettings.aiProvider,
      openaiApiKey: savedOpenAI || nextSettings.openaiApiKey,
      geminiApiKey: savedGemini || nextSettings.geminiApiKey,
      apifyApiKey: savedApify || nextSettings.apifyApiKey,
      aiKeys: {
        ...nextSettings.aiKeys,
        openai: savedOpenAI || nextSettings.aiKeys.openai,
        gemini: savedGemini || nextSettings.aiKeys.gemini,
      },
    });
  ...
  function saveFlatSettings() {
    const geminiKey = (settings.aiKeys.gemini || settings.geminiApiKey || "").trim();
    const openaiKey = (settings.aiKeys.openai || settings.openaiApiKey || "").trim();
    const selectedEngine = settings.aiProvider === "gemini" ? "gemini" : "openai";

    localStorage.setItem("GEMINI_API_KEY", geminiKey);
    localStorage.setItem("OPENAI_API_KEY", openaiKey);
    localStorage.setItem("analysisEngine", selectedEngine);
    localStorage.setItem("APIFY_API_KEY", (settings.apifyApiKey || "").trim());

    setSettings((prev) => ({
      ...prev,
      aiProvider: selectedEngine,
      geminiApiKey: geminiKey,
      openaiApiKey: openaiKey,
      apifyApiKey: (prev.apifyApiKey || "").trim(),
      aiKeys: {
        ...prev.aiKeys,
        gemini: geminiKey,
        openai: openaiKey,
    ...
    try {
      const response = await fetch("/api/scrape", {
        ...
        headers: {
          ...
          ...(settings.apifyApiKey ? { "x-apify-key": settings.apifyApiKey } : {}),
        },
        body: JSON.stringify({
          username,
          dateRange,
  ...
  async function handleAnalyze(post: InstagramPost) {
    ...
    try {
      setError("");
      // Helper: treat literal 'undefined' / 'null' strings as missing
      ...
      // RULE 1: Gemini is ALWAYS required for transcription.
      const transcriptionApiKey = getStoredKey("geminiApiKey");
      if (!transcriptionApiKey) {
        setError("Gemini key missing. It is required as the default for video transcription.");
        setIsLoading(false);
        return;
      }

      // RULE 2: Selected LLM handles the analysis.
      let analysisApiKey = "";
      ...
      else analysisApiKey = transcriptionApiKey; // If Gemini is the active provider, reuse the key

      if (!analysisApiKey) {
        setError(`${provider} key missing. Please add it in Settings for video analysis.`);
        setIsLoading(false);
        return;
      }
  ...
  const selectedProviderKey = settings.aiKeys[settings.aiProvider];
      ...
      <div className="flex w-full min-h-screen text-[var(--text)] flex-col relative z-10">
          ...
          >
            ...
            <header className="mb-[32px]">
              ...
              <h1 className="font-['Syne'] font-[800] text-[clamp(32px,5vw,48px)] tracking-[-0.03em] leading-[1.0] mb-[10px]">
                <span className="text-[#F0F2F7] block">Omnichannel</span>
        ...
        {showSettings ? (
          <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
            <div className="w-full max-w-lg rounded-2xl border border-[#2c2c2e] bg-[#1c1c1e] p-6">
              <h3 className="text-lg font-semibold text-white">API Settings</h3>
              <p className="mt-1 text-xs text-gray-400">Model selection and API keys are saved only in your browser localStorage.</p>

              <label className="mt-4 block text-sm text-gray-300">
                Select AI Model
                <select
                  value={settings.aiProvider}
                  onChange={(event) => setSettings((prev) => ({ ...prev, aiProvider: event.target.value as LocalSettings["aiProvider"] }))}
                  className="mt-2 h-10 w-full rounded-lg border border-[#2c2c2e] bg-[#0a0a0a] px-3 text-sm text-gray-100 outline-none ring-gray-500 transition focus:ring-2"
                >
                ...
                <input
                  ...
                  onChange={(event) =>
                    setSettings((prev) => ({
                      ...
                      aiKeys: {
                        ...prev.aiKeys,
                        [prev.aiProvider]: event.target.value,
                      },
                    }))
                  }
                  className="mt-2 h-10 w-full rounded-lg border border-[#2c2c2e] bg-[#0a0a0a] px-3 text-sm outline-none ring-gray-500 transition focus:ring-2"
                  placeholder={PROVIDER_PLACEHOLDERS[settings.aiProvider]}
                />
              </label>
              ...
              <label className="mt-3 block text-sm text-gray-300">
                ...
                <input
                  ...
                  onChange={(event) => setSettings((prev) => ({ ...prev, apifyApiKey: event.target.value }))}
                  className="mt-2 h-10 w-full rounded-lg border border-[#2c2c2e] bg-[#0a0a0a] px-3 text-sm outline-none ring-gray-500 transition focus:ring-2"
              ...
              <div className="mt-5 flex justify-end gap-2">
                <button onClick={() => setShowSettings(false)} className="h-10 rounded-lg border border-[#2c2c2e] px-4 text-sm text-gray-200">
                  Cancel
                </button>
                <button
                  onClick={() => {
                  ...
                  className="h-10 rounded-lg bg-white px-4 text-sm font-semibold text-black"
                >
                  Save Keys
                </button>
              </div>
